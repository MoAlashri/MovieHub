...
import { FaStar, FaPlay, FaBookmark, FaSearch, FaFilter } from "react-icons/fa";
    ...
    async function fetchMovies() {
      ...
      try {
        ...
        if (search) {
          ...
          url = `https://api.themoviedb.org/3/discover/movie?api_key=${API_KEY}&sort_by=${sortBy}&page=${page}${selectedGenre ? `&with_genres=${selectedGenre}` : ""}`;
        }
      ...
      <div className="relative h-64 md:h-80 flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center px-4">
          ...
          <p className="text-gray-300 text-lg">
            Explore thousands of films from every genre
          </p>
        </div>
      </div>
        ...
        <form onSubmit={handleSearch} className="flex gap-3 mb-8">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-11 pr-4 py-3.5 bg-white/5 border border-white/10 rounded-xl focus:outline-none focus:border-primary/60 focus:bg-white/10 transition-all text-white placeholder-gray-500"
            />
          </div>
          <button
            type="submit"
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {movies.map((movie) => (
                <motion.div
                  key={movie.id}
                  className="relative group rounded-xl overflow-hidden cursor-pointer hover:ring-2 hover:ring-primary/50 transition-all duration-300 shadow-md hover:shadow-xl hover:shadow-black/50"
                  whileHover={{ y: -4 }}
                >
                  {movie.poster_path ? (
                    <img
